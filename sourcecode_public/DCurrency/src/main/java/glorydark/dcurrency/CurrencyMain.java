...
public class CurrencyMain extends PluginBase {
    ...
    public static CurrencyMain mainPlugin;
    ...
    public static void loadRedeemsList(){
        File dic = new File(path+"/redeems/");
        if(!dic.exists()){
            if(dic.mkdirs()){
                mainPlugin.getLogger().info(getLang("tips_createFolder_fail"));
            }
            return;
        }
        for(File file: Objects.requireNonNull(dic.listFiles())){
            ...
            mainPlugin.getLogger().info(getLang("tips_loadRedeem_success", identifier));
        }
    }
}